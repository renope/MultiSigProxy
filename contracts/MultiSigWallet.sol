...
contract MultiSigWallet is ERC721Holder {
    ...
    modifier onlyWallet() {
        if (msg.sender != address(this))
            revert("error onlyWallet!");
        _;
    }

    modifier ownerDoesNotExist(address owner) {
        if (isOwner[owner])
            revert("error ownerDoesNotExist!");
        _;
    }

    modifier ownerExists(address owner) {
        if (!isOwner[owner])
            revert("error ownerExists!");
        _;
    }

    modifier transactionExists(uint transactionId) {
        if (transactions[transactionId].destination == address(0))
            revert("error transactionExists!");
        _;
    }

    modifier confirmed(uint transactionId, address owner) {
        if (!confirmations[transactionId][owner])
            revert("error confirmed!");
        _;
    }

    modifier notConfirmed(uint transactionId, address owner) {
        if (confirmations[transactionId][owner])
            revert("error notConfirmed!");
        _;
    }

    modifier notExecuted(uint transactionId) {
        if (transactions[transactionId].executed)
            revert("error notExecuted!");
        _;
    }
